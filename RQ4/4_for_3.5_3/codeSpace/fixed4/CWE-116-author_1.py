...
def list_files():
    ...
    # Sanitize the username to ensure it is alphanumeric
    sanitized_username = ''.join(char for char in username if char.isalnum())

    # Ensure sanitized username does not attempt directory traversal
    if '..' in sanitized_username or '/' in sanitized_username:
        return Response("Invalid username!", status=400)
    
    # Construct the path using the sanitized username
    user_path = os.path.join(BASE_DIR, sanitized_username)

    # Prevent directory traversal, again ensuring strict containment within BASE_DIR
...
if __name__ == "__main__":
    # Run the application without debug mode for production security
    app.run(debug=False)